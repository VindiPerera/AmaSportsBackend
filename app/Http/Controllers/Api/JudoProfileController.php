...
use App\Http\Requests\Player\StoreJudoProfileRequest;
use App\Http\Resources\JudoProfileResource;
...
class JudoProfileController extends Controller
{
    use ApiResponse;
    ...
    public function show(Request $request): JsonResponse
    {
        ...
        $profile = $player->judoProfile()->with(['careerStats', 'recentFights'])->first();

        if (! $profile) {
            $profile = new JudoProfile(['player_id' => $player->id]);
    ...
    /**
     * PUT /player/judo-profile — the only place a `player_sports` row for
     * Judo gets created (see Phase 2 fix A3).
     */
    public function update(StoreJudoProfileRequest $request): JsonResponse
    {
        $player = Player::firstOrCreate(['user_id' => $request->user()->id]);
        $sport = Sport::where('slug', Sport::JUDO_SLUG)->firstOrFail();

        $profile = DB::transaction(function () use ($request, $player, $sport) {
            $profile = JudoProfile::updateOrCreate(
                ['player_id' => $player->id],
                $request->safe()->only(['born', 'age', 'height', 'weight', 'college_university', 'current_ranking'])
            );
        ...
        return $this->success(new JudoProfileResource($profile), 'Judo profile saved successfully.');
    }
    ...
    {
        $sport = Sport::where('slug', Sport::JUDO_SLUG)->first();
